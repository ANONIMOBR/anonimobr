	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor nao seja um menbro inativo ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} saiu um corno(a) 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '⚠️ bonito tentando usar comando de adiministrador! 😑',
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna um administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5521992132053@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'punheteiro pra porra em mano'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra 2d isso e serio mano 😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
